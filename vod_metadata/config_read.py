...
def parse_config(config_path):
    ...
    provider_id = (
        config["VOD"].get("provider_id", "example.com").lower().strip()
    )
    ...
    if (
        ...
        raise ConfigurationError(
            "Configuration file error: provider_id must be must be a "
            "lower-case domain name up to 20 characters"
        )
    ...
    title_category = (
        config["VOD"].get("title_category", "Testing/Videos").strip()
    )
